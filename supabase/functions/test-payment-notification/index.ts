...
Deno.serve(async (req) => {
  ...
  try {
    // Use a simple secret check instead of JWT for this test-only function
    const ADMIN_SECRET = Deno.env.get('ADMIN_BOOTSTRAP_SECRET');
      ...
      try {
        const emailResp = await fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            ...
            html: `
              <h2>🧪 TESTE - Pagamento aprovado!</h2>
              <p><strong>Aluno:</strong> ${testData.userName}</p>
              <p><strong>Email:</strong> ${testData.userEmail}</p>
              <p><strong>Telefone:</strong> ${testData.userPhone}</p>
              <p><strong>Valor:</strong> R$ ${testData.amount}</p>
              <p><strong>Método:</strong> ${testData.method}</p>
              <p><strong>Tipo:</strong> ${testData.type}</p>
              <p><strong>Validade até:</strong> ${testData.expiration}</p>
              <p><strong>Data:</strong> ${timestamp}</p>
              <hr><p><em>Esta é uma notificação de teste.</em></p>
            `,
          }),
        });
    ...
    // Test WhatsApp via Twilio Gateway
    ...
    if (LOVABLE_API_KEY && TWILIO_API_KEY && TWILIO_FROM) {
      ...
      results.whatsapp = 'skipped: Twilio not fully configured';
    }

    console.log('Test notification results:', JSON.stringify(results));
